...
const withRegions =
  ...
  (props: Omit<SelectDepartmentsMenuProps, 'regions'>) => {
...
export const SelectDepartmentsMenu = withRegions(
  ({
    ...
    const someDepartmentsSelected = !!allDepartments.find((_) =>
      indexValues.has(_),
    )
    const { m } = useI18n()
    ...
    const onSelectAll = () => {
      allDepartments.forEach(
        allDepartmentsSelected ? indexValues.delete : indexValues.add,
      )
      onChange(indexValues.toArray())
    }
    ...
    return (
      <Menu anchorEl={anchorEl} open={open} onClose={onClose}>
        ...
        {regions.map((region) => {
          const allChecked = region.departments.every((_) =>
            indexValues.has(_.code),
          )
          const atLeastOneChecked = !!region.departments.find((_) =>
            indexValues.has(_.code),
          )
          return [
            <MenuItem
              ...
              key={region.label}
              onClick={() =>
                onSelectDepartments(region.departments.map((_) => _.code))
              }
            >
              <Checkbox
