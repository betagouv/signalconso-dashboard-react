    ...
    <ScDialog
      ...
      content={close => (
        <>
          ...
          <Controller
            ...
            render={({field}) => (
              <>
                <SelectCompany siret={field.value?.siret} onChange={field.onChange} openOnly={true} />
                {errors.company && (
      ...
      onConfirm={(e, close) => {
        handleSubmit((form: Form) => {
          const {siret, name, address, activityCode, isOpen, isHeadOffice, isPublic} = form.company
          if (name && address && siret) {
            _websiteCreate.mutate({
              host: form.host,
              company: {siret, name, address, activityCode, isOpen, isHeadOffice, isPublic},
            })
            close()
          } else {
