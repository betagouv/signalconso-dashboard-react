...
export const UsersAgentListPending = () => {
  ...
  const [selectedRole, setSelectedRole] = useState<RoleAgents[]>(
    roleAgents.map((_) => _),
  )

  const _agentPending = useGetAgentPendingQuery(
    selectedRole.length === 1 ? selectedRole[0] : undefined,
  )
  ...
  return (
    <>
      <Datatable
        ...
        superheader={
          <p>
            Cette page liste les agents à qui un mail d'invitation a été envoyé
            pour créer leur compte sur SignalConso, et qui ne l'ont pas encore
            utilisé.
          </p>
        }
        actions={
