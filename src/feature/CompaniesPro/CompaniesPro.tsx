...
function CompaniesProRow({
  ...
  return (
    <div className="lg:grid lg:grid-cols-2 py-2">
      <div className="">
        <NavLink to={siteMap.logged.company(_.id).stats.value} className="text-lg text-scbluefrance">
          {_.name}
        </NavLink>
      ...
      <div className="flex flex-col items-end justify-between pb-2">
        ...
        <div className="flex  justify-end gap-2">
          {_.level === AccessLevel.ADMIN && (
            <Btn href={'#' + siteMap.logged.company(_.id).accesses.value} variant="text" size="small" icon="group">
              {m.handleAccesses}
            </Btn>
          )}
          <Btn href={'#' + siteMap.logged.company(_.id).stats.value} variant="text" size="small" icon="query_stats">
            {m.myStats}
          </Btn>
          <Btn
